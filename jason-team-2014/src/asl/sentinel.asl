...
//+!select_goal: is_buy_goal(What) <- !init_goal(buy(What)).  //BUY STRATEGY  NOVO
...
+!buildIslands(S):
	not hill(_) & goalState(_,defineInitialPivots,_,_,satisfied) & (not lastCalcIsland(_) & S >= 23 | lastCalcIsland(N) & S - N >= 23) & .my_name(MyName) & play(MyName,sentinelLeader,"grMain")
<-	
	!determineIslands;
	-+lastCalcIsland(S).
-!buildIslands[error(ErrorCode),error_msg(MsgError)] <-
	.print("Error occurred to build islands! ", ErrorCode, " -> ", MsgError).
+!buildIslands(S).
