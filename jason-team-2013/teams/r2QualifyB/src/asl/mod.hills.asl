...
at_the_hill :- hill(Neighborhood) & position(MyV) & myTeam(MyTeam) &
				(
					.count(.member(V, Neighborhood) & ia.vertex(V, MyTeam), TotalOwned) &
					.length(Neighborhood, Total) &
					TotalOwned >= Total * 0.50 &
...
+!callGroupToHill(Neighborhood, Group):
 ...
 <-	
 	.print("#HILL# Calling group ", Group, " ",  SetAgs, " to ", Neighborhood);
