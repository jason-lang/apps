...
+!distributeAimVertices([], [island(VIsland, VerticesIsland, ValueIsland) | TI], [A1 | TA]): //There is no pivot
	not hill(_)
<-
	.print("#DISTRIBUTE# There is no pivots ", VIsland, " ", A1);
	.send(A1, achieve, gotoIslandVertex(VIsland, island(VIsland, VerticesIsland, ValueIsland)));
	!addIsland(VerticesIsland);
	!distributeAimVertices([], TI, TA).
...
<-
	.print("#DISTRIBUTE# There is no islands ", V1, " ", V2, " ", A1, " ", A2);
