...
+!select_goal: zoneScore(ZoneScore) & ZoneScore < 20 & there_is_disabled_friend_nearby(V)
		<-
		.print("Goto to be repair some agent (nearby)...");  
		!init_goal(goto(V)).
...
+!canHelp(AgentName, Pos):
    not busy(_) & position(MyPos) & (not zoneScore(_) | zoneScore(ZoneScore) & ZoneScore < 20)
<-
...
+!evaluateDisabledFriendNearby:
	...
	position(MyV) & 
	myNameInContest(MyName) & 
	zoneScore(ZoneScore) & ZoneScore >= 20 &
	.setof(AgentName,
				ia.edge(MyV,V,_) & 
				visibleEntity(Entity, V, MyTeam, disabled) &
				Entity \== MyName &
				friend(AgentName, Entity, _, _),
			AllAgents) & .length(Options, TotalOptions) & TotalOptions > 0
<-
	.send(AllAgents, tell, canCome).
+!evaluateDisabledFriendNearby.	