...
is_inspect_goal(V, Entity) :- not is_disabled & position(MyV) & myTeam(MyTeam) & visibleEntity(Entity, V, Team, _) & Team \== MyTeam & ia.edge(MyV,V,_) 
                  	& not entityType(Entity, _, _, _, _).
                  
is_inspect_goal(Entity) :- not is_disabled & position(MyV) & myTeam(MyTeam) & visibleEntity(Entity, MyV, Team, _) & Team \== MyTeam
				  & not entityType(Entity, _, _, _, _).
...
+!select_goal: is_inspect_goal(Entity)
		<-
		.print("I'm going to inspect ", Entity); 
		!init_goal(inspect).
...
+!select_goal: is_inspect_goal(Op, Entity)
		<-
		.print("I'm going to ", Op, " to inspect ", Entity); 
		!init_goal(goto(Op)).
...
+entityType(Entity, Type, MaxHealth, Strength, VisRange)[source(Source)] <-
	.print("Percept entity ", Entity, " ", Type, " ", MaxHealth, " ", Strength, " ", VisRange, " ", Source).
	
-entityType(Entity, Type, MaxHealth, Strength, VisRange)[source(Source)] <-
	.print("UnPercept entity ", Entity, " ", Type, " ", MaxHealth, " ", Strength, " ", VisRange, " ", Source).
