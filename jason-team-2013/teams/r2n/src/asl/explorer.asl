...
+!buildAreas(S):
	S <= 135 & (not lastCalcCoverage(_) & S >= 7 | lastCalcCoverage(N) & S - N >= 13) & .my_name(MyName) & play(MyName,explorerLeader,"grMain")
<-	
	!determineHills;
	-+lastCalcCoverage(S).
+!buildAreas(S):
	S > 135 & .my_name(MyName) & play(MyName,explorerLeader,"grMain") & hill(_) &
	.findall(Agent, friend(Agent, _, _, _), SetAgents)
<-	
