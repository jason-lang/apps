...
public class Graph {
	...
	public static final int MAXEDGES = 3000;
	...
	public List<String> getAllPivots(int amount) {
		LinkedList<String> result = null;
		
		PivotAlgorithm p = new PivotAlgorithm();
		List<PairPivot> resultPivots = p.execute(this, amount);
		
		result = new LinkedList<String>();
		
		if(resultPivots != null) {
			for (PairPivot pair : resultPivots) {
				result.addFirst(integer2vertex[pair.getMainVertex()]);
				result.addFirst(integer2vertex[pair.getAuxVertex()]);
			}
		}
