 ...
 */
there_is_enemy_nearby(Op) :- not is_disabled & position(MyV) & myTeam(MyTeam) & maxWeight(INF) & energy(MyE) & costAttack(CostAttack) &
...
there_is_enemy_nearby(Op) :- not is_disabled & position(MyV) & myTeam(MyTeam) & infinite(INF) & energy(MyE) & costAttack(CostAttack) &
                           .setof(V, ia.edge(MyV,V,W) & 
                                        W \== INF & 
                                        visibleEntity(Entity, V, Team, normal) & 
                                        Team \== MyTeam & 
 ...
 */
there_is_enemy_nearby(Op) :- not is_disabled & position(MyV) & myTeam(MyTeam) & maxWeight(INF) &
                           .setof(V, ia.edge(MyV,V,W) & 
                                    W \== INF & 
                                    visibleEntity(Entity, V, Team, normal) & 
                                    Team \== MyTeam & 
                           ...
                           & .length(Options, TotalOptions) & TotalOptions > 0 &
                           .nth(math.random(TotalOptions), Options, Op).
there_is_enemy_nearby(Op) :- not is_disabled & position(MyV) & myTeam(MyTeam) & infinite(INF) &
                           .setof(V, ia.edge(MyV,V,W) & 
                                    W \== INF & 
                                    visibleEntity(Entity, V, Team, normal) & 
                                    Team \== MyTeam &
...
//+!select_goal: get_vertex_to_go_attack_search(D, Path) <- !init_goal(goto(Op)).
