...
public class MyHonestAgent extends Agent {

	protected Map<String,Double>          reputations;
	protected Map<String,Map<Era,Double>> certainties;
	protected List<ReputationRequestMsg>  reputationRequestsToAccept;
	protected List<OpinionRequestMsg>     opinionRequests;
    ...
    public void prepareOpinionRequests() {
		// first collect the certainty answers
	    ...
	    Collections.shuffle(agentNames); // to change the agent to whom to ask 
        for (AppraisalAssignment assignment: assignedPaintings) {
            int nSent = 0;
            for (String agentToAsk: agentNames) {
                if (!agentToAsk.equals(getName())) {
