 ...
 */
     ...
     * 
     *  If the type is pong, it only processes the reception of it.
     * 
     ...
     * @see #processPong(String) processPong
     */
    ...
    /**
     * Sends a ping message to the server. For network state testing functions only. 
     * @param ping
     * @throws IOException
     */
    public void sendPing(String ping) throws IOException {
        ...
        payload.setAttribute("value",ping); //Long.toString(System.currentTimeMillis())); // sends the timestamp as a payload value. could also be anything else.
